---
id: deserialization-risk-finder
name: Deserialization Risk Finder
...
tags:
  ...
  - soc-2
summary: |
  This Codex skill detects insecure deserialization vulnerabilities across Java, .NET, Python, Ruby, PHP, and Node.js codebases — including the well-known gadget-chain classes (Java `ObjectInputStream`, .NET `BinaryFormatter`, Python `pickle`, Ruby `Marshal`, PHP `unserialize`, Node.js `node-serialize`).
